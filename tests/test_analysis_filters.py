...
def test_savitzky_golay_filter(sample_data: xr.DataArray):
    result = savitzky_golay_filter(sample_data, window_length=4, polyorder=1, dim="x")
    assert isinstance(result, xr.DataArray)
    assert result.shape == sample_data.shape


def test_savgol_filter_multi(sample_data: xr.DataArray):
