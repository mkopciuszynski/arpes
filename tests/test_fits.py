...
def test_plot_fit_creates_axes(mock_model_result) -> None:
    ax = plot_fit(mock_model_result)
    assert isinstance(ax, Axes)


def test_plot_fit_uses_provided_axes(mock_model_result) -> None:
    fig, ax = plt.subplots()
    result_ax = plot_fit(mock_model_result, ax)
    assert result_ax is ax


def test_plot_fit_no_exceptions(mock_model_result) -> None:
    try:
        plot_fit(mock_model_result)
    except Exception as e:
        pytest.fail(f"plot_fit raised an exception: {e}")


def test_plot_fits_creates_axes(mock_model_results):
